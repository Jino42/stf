...
__kernel void count(
    ...
    uint size = (nkeys / n_groups) / local_size;
    //Calculate where to start on the global array
    uint start = global_id * size;
    ...
    for (i = 0; i < BUCK; i++) {
        ...
        uint from = i * local_size + local_id;
...
__kernel void scan2(__global uint *input,
                    __global uint *output,
                    __local uint *local_scan,
                    __global uint *block_sum,
                    uint doBlockSum) {
...
/** COALESCE KERNEL **/
__kernel void coalesce(__global uint *scan,
...
/** REORDER KERNEL **/
__kernel void reorder(__global uint *array,
                      __global uint *array2,
                      __global uint *histo, // Scan Buffer
                      __global uint *output,
                      __global uint *output2,
                      const uint pass,
                      const uint nkeys,
                      __local uint *local_histo) {
    uint work_dim = get_work_dim();
    ...
    for (i = 0; i < size; i++) {
        uint item = array[i + start];
        uint item2 = array2[i + start];

        uint key = (item >> (pass * RADIX)) & (BUCK - 1);
...
__kernel void parallelcmp(const __global uint *input,
                          __global uint *output,
                          const uint nkeys) {
    uint g_id = (uint)get_global_id(0);
    uint l_size = (uint)get_local_size(0);
...
__kernel void reduce(__global uint *input,
                     __global uint *output,
                     const uint nkeys) {
    uint g_id = (uint)get_global_id(0);
    uint l_id = (uint)get_local_id(0);
