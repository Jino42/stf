...
WidgetEditor::WidgetEditor() : AWidget("Editor", NTL_IMGUI_WINDOW_NO_FLAGS) {}
...
void WidgetEditor::beginContent_() {
  ...
  ImGui::SliderScalar("Max",   ImGuiDataType_Float,  &timerSpeed, &u32_zonce,
  &u32_max); if (DisplayWindow::Get().getKey(GLFW_KEY_3) == KeyState::kDown) {
...
*/
  ...
  if (ImGui::CollapsingHeader("Required")) {
      ...
      static unsigned int newMax =
  MainGraphic::Get().particleSystem_.getEmitter("Lol").getNbParticleMax();
      static unsigned int newPerSec =
  MainGraphic::Get().particleSystem_.getEmitter("Lol").getNbParticlePerSec();
      ImGui::SliderScalar("Max",   ImGuiDataType_U32,  &newMax, &u32_zonce,
  &u32_max);
      //NeedToReset = true
      ImGui::SliderScalar("Sec",   ImGuiDataType_U32,  &newPerSec, &u32_zonce,
  &u32_max); if (DisplayWindow::Get().getKey(GLFW_KEY_T)) {
